 ...
 *
 * `formatos` existe porque nem toda imagem do site pode ser servida em AVIF, e
 * gerar o que ninguém referencia é peso publicado à toa. Só leva AVIF quem entra
 * num <picture> de verdade; o resto é WebP, que é universal desde 2020.
 */
const FONTES = [
  ...
  // Só WebP: estas três não estão num <picture>. O JS troca a foto em cena pelo
  // src/srcset do <img>, e um <source> venceria essa troca.
  ['img/founders.jpg', [800, 1200], ['webp']],
  ['img/joene.jpg', [800, 1200], ['webp']],
  ['img/dayana.jpg', [800, 1200], ['webp']],
  ...
  // Pôsteres dos vídeos da faixa. O atributo poster= aceita um único URL e não
  // negocia formato — só WebP, nunca AVIF.
  ['img/real/clip-06.jpg', [680], ['webp']],
  ['img/real/clip-07.jpg', [640], ['webp']],
  ['img/real/clip-08.jpg', [640], ['webp']],
  ['img/real/clip-09.jpg', [640], ['webp']],
  ['img/real/clip-10.jpg', [640], ['webp']],
  ['img/real/clip-11.jpg', [640], ['webp']],
  ['img/real/clip-13.jpg', [576], ['webp']],
...
for (const [origem, larguras, formatos = ['avif', 'webp']] of FONTES) {
  ...
  for (const w of larguras) {
    ...
    for (const fmt of formatos) {
      ...
      if (w === Math.max(...larguras.filter(x => x <= meta.width)) && fmt === formatos[0]) totalDepois += info.size;
 ...
 */
{
  ...
  // Limiar de 2%: sem ele, rodar o script de novo reescreve o arquivo por alguns
  // bytes de diferença e polui o diff a cada build.
  if (buf.length < antes * 0.98) {
...
console.log(`Melhor formato, maior largura:${(totalDepois / 1024).toFixed(1).padStart(8)} KB   (−${Math.round((1 - totalDepois / totalAntes) * 100)}%)`);
if (avisos.length) {
